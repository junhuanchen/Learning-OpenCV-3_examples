...
int main( int argc, const char** argv )
{
	...
	/************************************************************************/
  {
    ...
    cv::Mat src = imread(argv[1],IMREAD_GRAYSCALE);
    cv::Mat dst;
    ...
    cv::Mat gray = imread(argv[1],IMREAD_GRAYSCALE);//a bike in gray
    GaussianBlur( gray, gray, Size(9, 9), 2, 2 );
    Mat temp;
    vector<Vec4i> linesP;
    vector<Vec3f> circles;
    //first find the canny edge
    Canny(gray,temp,50,200);
    //find lines
    ...
    {
      Vec4i l = linesP[i];
      line(gray,Point(l[0],l[1]),Point(l[2],l[3]),Scalar(255));
    }
